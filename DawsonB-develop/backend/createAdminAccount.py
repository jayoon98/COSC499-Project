...
createAdminAccount_blueprint = Blueprint('createAdminAccount_blueprint', __name__)

@createAdminAccount_blueprint.route('/createAdminAccount', methods=['POST'])
def createAdminAccount():
    ...
    if fetchedEmails is None:
        ...
        query = "INSERT INTO users (email, pass_hash, first_name, last_name, client, admin) VALUES ('%s', '%s','%s', '%s', %s, %s)"
        cursor.execute(query % (email, hashedPassword, first_name, last_name, 0, isAdmin))
