...
export function ReportDetails(props: ReportDetailsProps) {
	...
	const displaydata = (domainarray: any[]) => {
		...
		console.log("displaydata : " + domainarray)

		const label: string[] = domainarray.map((index) => {
			return index.toString()
		});
		const datas: number[] = domainarray.map((value: number) => { return value });
		console.log("displaydata label: " + label)
		console.log("displaydata data: " + datas)
		const displaydata2 = {
			labels: label,
			datasets: [
				{
